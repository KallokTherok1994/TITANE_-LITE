...
tar -czvf TITANE_LITE_backup_$(date +%Y%m%d_%H%M%S).tar.gz TITANE_LITE/

# Vérifier l'intégrité
tar -tzvf TITANE_LITE_backup_*.tar.gz | tail -20
```
...
```bash
# Option A: SCP/SFTP
scp TITANE_LITE_backup_*.tar.gz user@new-machine:/home/user/Documents/

# Option B: rsync (recommandé pour gros volumes)
rsync -avzP --delete TITANE_LITE/ user@new-machine:/home/user/Documents/TITANE_LITE/

# Option C: Git (si remote configuré)
cd TITANE_LITE
git push origin MAIN
# Sur nouvelle machine:
git clone <repo-url> TITANE_LITE
```
...
tar -xzvf TITANE_LITE_backup_*.tar.gz
...
cd /home/user/Documents/TITANE_LITE/src-tauri
