...
| `TITANE_LITE-main.zip` | 98 Mo | ⚠️ Archive versionnée | REMOVE |
...
tar -czf TITANE_LITE_BACKUP_$(date +%Y%m%d_%H%M%S).tar.gz TITANE_LITE/
...
```bash
/home/titane/Documents/TITANE_LITE_BACKUP_<timestamp>.tar.gz
```
...
cd /home/titane/Documents/
rm -rf TITANE_LITE/
tar -xzf TITANE_LITE_BACKUP_<timestamp>.tar.gz
```
