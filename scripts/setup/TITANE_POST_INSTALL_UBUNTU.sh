...
#   7. Clonage et configuration TITANE_LITE
...
TITANE_REPO="git@github.com:KallokTherok1994/TITANE_LITE.git"
...
# ═══════════════════════════════════════════════════════════════════════════════
# PHASE 7 : CLONAGE TITANE_LITE
# ═══════════════════════════════════════════════════════════════════════════════

log_header "PHASE 7 : CLONAGE TITANE_LITE"
...
if [ -d "$PROJECTS_DIR/TITANE_LITE" ]; then
    log_info "TITANE_LITE existe déjà"
    log_step "Mise à jour du repository..."
    cd "$PROJECTS_DIR/TITANE_LITE"
    git pull origin main 2>&1 | tee -a "$LOG_FILE" || log_warning "Pull échoué - vérifiez manuellement"
else
    log_step "Clonage de TITANE_LITE..."
    ...
    if ssh -T git@github.com 2>&1 | grep -q "successfully authenticated"; then
        ...
        git clone "https://github.com/KallokTherok1994/TITANE_LITE.git" 2>&1 | tee -a "$LOG_FILE"
...
echo "   $ cd $PROJECTS_DIR/TITANE_LITE"
echo ""
