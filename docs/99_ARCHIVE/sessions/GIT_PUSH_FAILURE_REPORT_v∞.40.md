...
1. **Backup actuel**
   ...
   tar -czf TITANE_LITE_BACKUP_v∞.40_$(date +%Y%m%d).tar.gz TITANE_LITE/
   ```

2. **Créer nouveau repo propre**
   ```bash
   mkdir TITANE_LITE_CLEAN
   cd TITANE_LITE_CLEAN
   git init
   git remote add origin https://github.com/KallokTherok1994/TITANE_LITE.git
   ```
...
# 1. Backup
cd /home/titane/Documents
tar -czf TITANE_LITE_BACKUP_v∞.40_20251205.tar.gz TITANE_LITE/
