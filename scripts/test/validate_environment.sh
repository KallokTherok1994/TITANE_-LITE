...
if [ "$FAIL_COUNT" -gt 0 ] || [ "$WARN_COUNT" -gt 0 ]; then
    ...
    if ! [ -d "$PROJECTS_DIR/TITANE_LITE" ]; then
        echo "  • Cloner TITANE_LITE:"
        echo "    git clone git@github.com:KallokTherok1994/TITANE_LITE.git ~/Projets/TITANE_LITE"
        echo ""
    fi
    
    if [ -d "$PROJECTS_DIR/TITANE_LITE" ] && ! [ -d "$PROJECTS_DIR/TITANE_LITE/node_modules" ]; then
        echo "  • Installer les dépendances du projet:"
        echo "    cd ~/Projets/TITANE_LITE && pnpm install"
        echo ""
    fi
fi
