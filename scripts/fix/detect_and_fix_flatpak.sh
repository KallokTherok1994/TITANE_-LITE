#!/usr/bin/env bash
# TITANE_LITE v∞ — Détection et Résolution Automatique Environnement Flatpak
...
if grep -qi "freedesktop" /etc/os-release 2>/dev/null; then
    ...
    echo ""
    echo -e "      ${GREEN}cd ~/Documents/TITANE_LITE${NC}"
...
if [ ! -f "package.json" ]; then
    ...
    echo "Assurez-vous d'être dans le dossier racine du projet TITANE_LITE"
    exit 1
fi
