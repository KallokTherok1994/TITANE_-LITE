...
└── TITANE_LITE_PREDEPLOY_v12.sh  # Script pré-déploiement automatisé
...
**`TITANE_LITE_PREDEPLOY_v12.sh`** (17 KB, 7 étapes) :
...
**Usage** :
```bash
chmod +x TITANE_LITE_PREDEPLOY_v12.sh
./TITANE_LITE_PREDEPLOY_v12.sh
```
...
| `TITANE_LITE_PREDEPLOY_v12.sh` | Créé | Script pré-déploiement automatisé (7 étapes) |
...
2. **Exécuter** `./TITANE_LITE_PREDEPLOY_v12.sh` pour build final
