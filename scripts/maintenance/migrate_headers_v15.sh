...
migrate_header() {
    ...
    if grep -q "TITANE.*v14" "$file" 2>/dev/null; then
        ...
        sed -i 's/TITANE_LITE v14/TITANE∞ v15/g' "$file"
        updated=true
    fi
    ...
    if grep -q "TITANE.*v13" "$file" 2>/dev/null; then
        ...
        sed -i 's/TITANE_LITE v13/TITANE∞ v15/g' "$file"
        updated=true
    fi
    ...
    if grep -q "TITANE.*v12" "$file" 2>/dev/null; then
        ...
        sed -i 's/TITANE_LITE v12/TITANE∞ v15/g' "$file"
        updated=true
    fi
    ...
    if grep -q "TITANE.*v17" "$file" 2>/dev/null; then
        ...
        sed -i 's/TITANE_LITE v17[^ ]*/TITANE∞ v15/g' "$file"
        updated=true
    fi

    # Frontend TypeScript: v13 → v15
    if grep -q "TITANE_LITE v13" "$file" 2>/dev/null; then
        sed -i 's/TITANE_LITE v13/TITANE∞ v15/g' "$file"
        updated=true
    fi
