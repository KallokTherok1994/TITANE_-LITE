...
- `pnpm run verify` script de validation unifié (lint + check + test + e2e + rust)
...
- 🔴 **CRITICAL**: User must open native terminal (not VS Code Flatpak)
  - Action: Super → "Terminal" → `cd ~/Documents/TITANE_LITE && ./detect_and_fix_flatpak.sh`
